...
# INFO: whole calling workflow of attdeepcaller
...
attdeepcaller="${SHELL_FOLDER}/../attdeepcaller.py"
...
${PYTHON} ${attdeepcaller} CheckEnvs \
...
"${PYTHON} ${attdeepcaller} CallVariantsFromCffi \
...
then
    ...
    gzip -fdc ${OUTPUT_FOLDER}/pileup.vcf.gz | ${PYPY} ${attdeepcaller} SelectQual --phase --output_fn ${PHASE_VCF_PATH} --var_pct_phasing ${PHASING_PCT}
...
gzip -fdc ${OUTPUT_FOLDER}/pileup.vcf.gz | ${PYPY} ${attdeepcaller} SelectQual --output_fn ${CANDIDATE_BED_PATH} \
...
"${PYPY} ${attdeepcaller} SelectCandidates \
