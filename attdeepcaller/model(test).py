...
from clair3.myattention import get_activations
...
def attention_3d_block(inputs):
    ...
    a = tf.keras.layers.Dense(input_dim, activation='softmax')(a)
    if SINGLE_ATTENTION_VECTOR:
        a = Lambda(lambda x: K.mean(x, axis=1), name='dim_reduction')(a)
        a = RepeatVector(input_dim)(a)
    a_probs = tf.keras.layers.Permute((1, 2), name='attention_vec')(a)

    output_attention_mul = tf.keras.layers.concatenate([inputs, a_probs])
...
class Clair3_P(tf.keras.Model):#Clair3_P函数是用来生成pileup模型的
    ...
    def __init__(self, add_indel_length=False, predict=False):
        ...
        self.Con2D = tf.keras.layers.Conv1D(filters = 2, kernel_size = 1, activation = 'relu')(tensorflow.keras.layers.Input(shape=(33,32)))
        self.Dropout1  =tf.keras.layers.Dropout(0.3)(self.Con2D)

        self.LSTM1 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
            units=self.LSTM1_num_units,#128units
            return_sequences=True,
            kernel_regularizer=L2_regularizers
        ))(self.Dropout1)
        #units 输出空间的维度
        #return_sequences是否返回最后一个输出。在输出序列或完整序列中
        #kernel_regularizer=None,＃正则化函数应用于kernel权重矩阵

        self.LSTM2 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
            units=self.LSTM2_num_units,#160units
            return_sequences=True,
            kernel_regularizer=L2_regularizers
        ))(self.LSTM1)
        ...
        self.L3_dropout = tf.keras.layers.Dropout(rate=self.L3_dropout_rate)(self.LSTM2)
        # # 方法二：使用one_hot来进行加减运算
        # self.L3_dropout = tf.constant([x for x in range(1, 3)])
        # i = 1
        # # 生成一个one_hot张量，长度与tensor_1相同，修改位置为1
        # shape = self.L3_dropout.get_shape().as_list()
        # one_hot = tf.one_hot(i, shape[0], dtype=tf.int32)
        # # 做一个chen法运算，将one_hot为一的变为原张量该位置的值进行相减
        # self.L3_dropout2 = self.L3_dropout + self.L3_dropout[i] * one_hot




        self.attention3 = attention_3d_block2(self.L3_dropout)*(self.L3_dropout)

        self.L3_dropout_flatten = tf.keras.layers.Flatten()(self.attention3)

        self.L4 = tf.keras.layers.Dense(units=self.L4_pileup_num_units, activation='selu',kernel_regularizer=L2_regularizers)(self.L3_dropout_flatten)

        self.L4_dropout = tf.keras.layers.Dropout(rate=self.LSTM2_dropout_rate, seed=param.OPERATION_SEED)(self.L4)

        self.L5_1 = tf.keras.layers.Dense(units=self.L5_1_num_units, activation='selu', kernel_regularizer=L2_regularizers)(self.L4_dropout)

        self.L5_1_dropout = tf.keras.layers.Dropout(rate=self.L5_1_dropout_rate, seed=param.OPERATION_SEED)(self.L5_1)

        self.L5_2 = tf.keras.layers.Dense(units=self.L5_2_num_units, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_1_dropout)

        self.L5_2_dropout = tf.keras.layers.Dropout(rate=self.L5_2_dropout_rate, seed=param.OPERATION_SEED)(self.L5_2)

        self.Y_gt21_logits = tf.keras.layers.Dense(units=self.output_gt21_shape, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_2_dropout)

        self.Y_genotype_logits = tf.keras.layers.Dense(units=self.output_genotype_shape, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_2_dropout)

        if self.add_indel_length:

            self.L5_3 = tf.keras.layers.Dense(units=self.L5_3_num_units, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_2_dropout)

            self.L5_3_dropout = tf.keras.layers.Dropout(rate=self.L5_3_dropout_rate, seed=param.OPERATION_SEED)(self.L5_3)

            self.L5_4 = tf.keras.layers.Dense(units=self.L5_4_num_units, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_3_dropout)

            self.L5_4_dropout = tf.keras.layers.Dropout(rate=self.L5_4_dropout_rate, seed=param.OPERATION_SEED)(self.L5_4)

            self.Y_indel_length_logits_1 = tf.keras.layers.Dense(units=self.output_indel_length_shape_1, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_4_dropout)

            self.Y_indel_length_logits_2 = tf.keras.layers.Dense(units=self.output_indel_length_shape_2, activation='selu', kernel_regularizer=L2_regularizers)(self.L5_4_dropout)
    ...
    def call(self, x,):
        ...
        x = tf.cast(x, tf.float32)#函数的作用是执行 tensorflow 中张量数据类型转换，比如把int8转换为float32。

        # x = self.Con2D(x)*(tensorflow.keras.layers.Input(shape=(33,32)))
        # x = self.Dropout1(x)
        ...
        x = self.L3_dropout(x)(x)

        # Attention
        x = self.attention3(x)*(x)

        x = self.L3_dropout_flatten(x)*(x)

        x = self.L4(x)*(x)

        x = self.L4_dropout(x)*(x)

        l5_1_dropout = self.L5_1_dropout(self.L5_1(x))*(x)

        l5_2_dropout = self.L5_2_dropout(self.L5_2(x))*(x)

        y_gt21_logits = self.softmax(self.Y_gt21_logits(l5_1_dropout))*(l5_1_dropout)

        y_genotype_logits = self.softmax(self.Y_genotype_logits(l5_2_dropout))*(l5_2_dropout)

        if self.add_indel_length:
            l5_3_dropout = self.L5_3_dropout(self.L5_3(x))*(x)

            l5_4_dropout = self.L5_4_dropout(self.L5_4(x))*(x)

            y_indel_length_logits_1 = self.softmax(self.Y_indel_length_logits_1(l5_3_dropout))*(l5_3_dropout)

            y_indel_length_logits_2 = self.softmax(self.Y_indel_length_logits_2(l5_4_dropout))*(l5_4_dropout)
...
class Clair3_F(tf.keras.Model):##Clair3_F函数是用来生成fulalign模型的
