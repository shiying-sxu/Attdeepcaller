...
from attdeepcaller.task.main import *
...
def get_training_array(tensor_fn, var_fn, bed_fn, bin_fn, shuffle=True, is_allow_duplicate_chr_pos=True, chunk_id=None,
    ...
    生成训练数组进行训练。这里带有 blosc:lz4hc 的 pytables 用于极快的压缩和解压缩，
    可以满足gpu利用率的要求。 lz4hc 解压允许将训练数组解压速度提高 4~5 倍
    使用tensorflow tfrecord文件格式，仅10G内存，当前gpu利用率就可以达到85%以上。
    tensor_fn：从 CreateTensorPileup 或 CreateTensorFullAlign 获取的字符串格式张量，包括 contig 名称位置、张量矩阵、替代信息。
    var_fn：来自 GetTruths 的简化变体 (vcf) 格式，其中包括重叠群名称、位置、参考碱基、替代碱基、基因型。
    bin_fn：pytables格式输出的bin文件名。
    shuffle：生成训练数据时是否应用索引shuffle，默认为True，提升鲁棒性。
    is_allow_duplicate_chr_pos：训练时是否允许重复位置，如果存在下采样数据，下深度会添加一个随机前缀字符。
    chunk_id：特定块 id 与总 chunk_num 一起用于并行执行。这里将合并所有带有 sampe 前缀的张量文件。
    chunk_num：并行执行的总块数。每个块引用一个较小的参考区域。
    平台：张量形状的平台，与pb和illumina相比，不会提供更大的最大深度。
    堆积：是否处于堆积模式。定义两种调用模式，堆积或完全对齐。
    maximum_non_variant_ratio: 定义训练的最大非变体比率，我们总是期望使用更多的非变体数据，同时它会大大增加训练
    时间，尤其是ont数据，这里我们通常使用1:1或者1:2的变体候选：非变体候选。
    Candidate_details_fn_prefix：一个计数器，用于根据替代文件中的信息计算总变体和非变体。
