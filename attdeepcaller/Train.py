...
import attdeepcaller.model as model_path
from shared.utils import str2bool

# 想让代码仅仅运行在CPU下，可在原代码中加入如下代码：
...
class FocalLoss(tf.keras.losses.Loss):
    ...
    更新版本的焦点损失函数，对于多类分类，我们删除了 alpha 参数，损失函数
     更稳定，并添加梯度裁剪，避免梯度爆炸和精度溢出。
    """
...
获取训练和验证数据的块列表。 我们将随机分割训练和验证数据集，所有训练数据直接从各种张量 bin 文件中获取。
    """
...
def train_model(args):
    ...
    max_epoch = 50 #args.maxEpoch if args.maxEpoch else param.maxEpoch  ####设置最大迭代数,原始值为30
    ...
    # Use first 20 element to initialize tensorflow model using graph model  使用前 20 个元素通过图形模式对tensorflow模型初始化
    output = model(np.array(table_dataset_list[0].root.position_matrix[:20]))

    logging.info(model.summary(print_fn=logging.info))####打印模型参数
    ...
    # model.save('sy.h5')####保存模型NotImplementedError: Saving the model to HDF5 format requires the model to be a Functional model or a Sequential model. It does not work for subclassed models, because such models are defined via the body of a Python method, which isn't safely serializable. Consider saving to the Tensorflow SavedModel format (by setting save_format="tf") or using `save_weights`.


def main():
    parser = ArgumentParser(description="Train a attdeepcaller model")
    ...
    # Tensor2Bin.py 生成的二进制张量输入，支持使用 pytables 的多个 bin 阅读器
